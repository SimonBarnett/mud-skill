...
1. **LOCKED — Connection** — Use only operator-supplied host/client/credentials;
   ...
   LOCKED — First room for guest `G` is the Mended Drum main bar (`discworld-location-mended-drum`). Do not copy the price list or sign text here. `look` at an exit previews and does not move you. From the main bar, `south` is the entrance; `north` returns to the bar. Do not take south or northwest from the entrance. The staircase toward the streets was not climbed. From the main bar, `up` is the landing; `down` returns. Do not copy the sign text here. North is a closed door. Do not open it from this playbook. West previews a shady stage used by minor villains. Do not enter it in the first 10 minutes. Alleys and lit streets apply only after the game shows a street.
